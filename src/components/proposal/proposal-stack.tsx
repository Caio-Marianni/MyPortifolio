...
export function ProposalStack({ image, wordmark, descriptor, script, scriptFont = "", wordmarkFont = "", descriptorFont = "", tab, corner, rail, strip, imagePosition, columns, children, overlay, priority }: ProposalContent) {
  ...
  return (
    ...
    <section className="grain-cream relative flex min-h-screen w-full flex-col overflow-hidden font-inter text-ink">
      ...
      <div className="relative isolate z-10 flex shrink-0 items-center justify-between overflow-hidden bg-chrome px-5 py-3 text-chrome-ink shadow-lg shadow-black/40">
        <LogoWatermark />
      ...
      {(railLeft.length > 0 || railRight.length > 0 || rail?.rating) && (
        ...
        <div className="flex shrink-0 items-center gap-3 overflow-hidden bg-chrome px-5 py-3 text-[12.5px] text-white/70">
      ...
      <div className="flex flex-1 basis-0 flex-col items-center justify-center px-5 py-8 z-10">
        ...
        <div className="mx-auto w-full max-w-[360px]">
          ...
          {columns && (
            ...
            <div
              className="mt-5 grid grid-cols-2 gap-x-3 gap-y-[0.6em] text-[13px] font-bold"
              ...
              {columns.map((column) => (
                <div key={column.link.href} className="grid grid-rows-subgrid" style={{ gridRow: `span ${gridRows}` }}>
                  {column.stats.map((stat, i) => (
                    ...
                    <span key={i} className="flex min-h-[1.3em] items-center justify-center text-center leading-[1.35] text-ink/55">
                      {stat}
                    </span>
                  ))}
                  <Link
      ...
      {strip && (
        ...
        <p
          aria-hidden
          className={`pointer-events-none absolute inset-x-0 -bottom-4 mx-1 origin-bottom translate-y-[0.14em] scale-y-[2]  select-none ${scriptFont} font-black text-[clamp(40px,8vw,150px)] uppercase leading-[0.95] bg-gradient-to-t from-ink/20 to-ink/0 bg-clip-text text-transparent [text-align-last:justify] [text-align:justify]`}
        >
          {strip.split("").join(" ")}
        </p>
      )}
